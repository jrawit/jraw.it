...
export function getPointsOnSmoothedPathQuadratic(
  ...
  for (let i = 0; i < points.length - 1; i++) {
    ...
    const startPoint =
      i === 0
        ? p0
        : { x: (points[i - 1].x + p0.x) / 2, y: (points[i - 1].y + p0.y) / 2 };
    const endPoint = midPoint;
    const controlPoint = p0;
    for (let t = 1; t <= density; t++) {
      const step = t / density;
      smoothedPoints.push(
        getPointOnQuadraticBezier(startPoint, controlPoint, endPoint, step)
      );
    }
    if (i === points.length - 2) {
      ...
      for (let t = 1; t <= density; t++) {
        const step = t / density;
        smoothedPoints.push(
          getPointOnQuadraticBezier(lastMid, lastControl, lastPoint, step)
        );
      }
      if (
...
export function isPointNearPolygonOutline(
  ...
  if (vertices.length >= 2) {
    for (let i = 0; i < vertices.length; i++) {
      ...
      const distSqSegment = distanceSqFromPointToSegment(
        point.x,
        point.y,
        p1.x,
        p1.y,
